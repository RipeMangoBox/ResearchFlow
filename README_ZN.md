...
### 面向论文建库、idea 生成与代码联动的 agent-ready 本地科研知识库

**把论文清单、PDF 与结构化分析笔记，沉淀成 Claude Code、Codex CLI 与其他 AI 科研工具可以共享的 research memory。**
...
ResearchFlow 用来把论文清单与 PDF 变成可被 agent 直接使用的本地科研知识库。
...
- Codex CLI：通过 `AGENTS.md` 与 `.agents/skills`
...
- `AGENTS.md` 负责告诉 Codex 这个仓库的工作流与 source-of-truth 目录
- `.agents/skills` 直接链接到 `.claude/skills`，因此无需复制一套 skill 定义

这样一来，同一个仓库就能同时服务 Claude Code 与 Codex CLI，而不是维护两套分裂的 prompt 系统。
...
- `scripts/` 存放维护和自动化脚本
- 想快速浏览 skill 路线图，可直接看 `.claude/skills/User_README.md`
...
- Obsidian 是可选的；即使只把仓库当普通本地目录给 agent 使用，也完全可行。仓库里的 `obsidian setting/` 是可公开分享的模板，已经去除了私有 workspace 状态和分享 token
