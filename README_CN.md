...
```text
collect / Zotero 同步 → download* → analyze → build index → research assist
                        (* Zotero 同步跳过 download — PDF 已在本地)
```
...
> 已经有 PDF？直接跳到 analyze。已经有 idea？直接跳到 focus 或 reviewer 压测。已经有 Zotero 文库？从 `papers-sync-from-zotero` 开始。不确定当前该用哪个 skill？先用 `research-workflow`。
...
| 🔄 Zotero 同步 | 从 Zotero 文库或本地 PDF 文件夹导入论文到 RF 结构，支持增量同步 | `papers-sync-from-zotero` |
...
- `analysis_log.csv` 的论文状态主线是 `Wait → Downloaded → checked`。Zotero 同步直接进入 `Downloaded`（PDF 已在本地）。异常状态为 `analysis_mismatch`（分析模板不完整）与 `too_large`（PDF 过大），旁支状态为 `Missing` 与 `Skip`。完整定义见 `.claude/skills/STATE_CONVENTION.md`。
